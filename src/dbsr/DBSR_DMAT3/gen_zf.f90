...
Subroutine Gen_zf
  ...
  Real(8) :: FL,FV, GFL,GFV, WL,WV, GWL,GWV, &
       alfaL, alfaV, alfL, alfV, de,   &
       au_cm, au_eV, S, g1,g2, ANGS, ANGSA , dmatL,dmatV
  Real(8), allocatable :: Cbra(:,:), Cket(:,:), SL(:,:), SV(:,:), E1(:), E2(:)
  ...
  allocate(Cbra(kdm1,mstate1),Cket(kdm2,mstate2))
  allocate(E1(mstate1),E2(mstate2))
  Cbra = 0
  Cket = 0
  ...
  allocate(SL(mstate1,mstate2),SV(mstate1,mstate2))
  SL = 0
  SV = 0
  ...
  Do j1=1,mstate1
     if(ctype1.eq.'j') then
        read(nub1,*) j,label1
        read(nub1,*) E1(j1),jot1,ic1,ic2
        read(nub1,*) Cbra(ic1:ic2,j1)
  ...
  Do j2=1,mstate2
     if(ctype2.eq.'j') then
        read(nub2,*) j,label2
        read(nub2,*) E2(j2),jot2,jc1,jc2
        read(nub2,*) Cket(jc1:jc2,j2)
  ...
  SL = matmul(transpose(Cbra(ic1:ic2,:)), matmul(dl(ic1:ic2,jc1:jc2),Cket(jc1:jc2,:)))
  if(ktype.ne.'M') then
     SV = matmul(transpose(Cbra(ic1:ic2,:)), matmul(dv(ic1:ic2,jc1:jc2),Cket(jc1:jc2,:)))
  end if
  ...
  Do j1=1,mstate1
     ...
     Do j2=1,mstate2
        ...
        de = abs(E2(j2)-E1(j1)); if(de.lt.eps_de) Cycle

        ! ... line strengths:
        if(SL(j1,j2).eq.0.0) Cycle

        if(kpol.eq.0) then
           write(nur,'(/i4,f14.8,2x,a)') jot1-1,E1(j1),Label1
           write(nur,'( i4,f14.8,2x,a)') jot2-1,E2(j2),Label2
           write(nur,'(a,2E12.5,f10.2)')  'S = ',SL,SL*SL,SL*SL*100
           Cycle
        end if

        dmatL = SL(j1,j2)
        dmatV = SV(j1,j2)/de*c_au
        ...
        GWL = S*SL(j1,j2); GFL = c_au**3/2*GWL/(de*de)
        GWV = S*SV(j1,j2); GFV = c_au**3/2*GWV/(de*de)

        g1 = jot1+1; g2 = jot2+1

        alfL = 2*dmatL*dmatL/(E2(j2)-E1(j1))/(kp*g1); alfaL = alfaL + alfL
        alfV = 2*dmatV*dmatV/(E2(j2)-E1(j1))/(kp*g1); alfaV = alfaV + alfV

        if(E2(j2).gt.E1(j1)) then
           FL = GFL/g1; WL = GWL/g2/time_au
           FV = GFV/g1; WV = GWV/g2/time_au
           write(nuf,'(/a,i2,f15.8,2x,a)') par1,jot1,E1(j1),trim(Label1)
           write(nuf,'( a,i2,f15.8,2x,a)') par2,jot2,E2(j2),trim(Label2)
        else
        ...
        if(ialpha.eq.0) then
           write(nuf,'(1x,a1,i1,2x,a4,1PD12.5,3x,a5,D12.5,3x,a6,D12.5)') &
                ktype,kpol,'S = ',SL(j1,j2),AA,FL,'AKI = ',WL
           if(SV(j1,j2).ne.0.d0) &
                write(nuf,'(9x,1PD12.5,8x,D12.5,9x,D12.5)') SV(j1,j2),FV,WV
        else
           ...
           if(SV(j1,j2).ne.0.d0) &
                ...
                SV(j1,j2),FV,WV, dmatV, alfV,alfaV
        end if

     End do
  End do   !  over solutions

  deallocate(SL,SV,Cbra,Cket)
