...
!----------------------------------------------------------------------
      ...
      Implicit none
      Integer, intent(in) :: kdn, iext, N1(kdn),N2(kdn)
      Integer :: N3(kdn),N4(kdn),N5(kdn),N6(kdn)
...
! ... define the cont.orb.
      ...
      elseif(kdn.eq.1) then                      ! <kl|nl>
       ...
       if(iext.gt.1) Call Stop_mpi(0,0,' Check_det: iext > 1')
       io=IBORT(N1(1),N2(1))
      ...
      elseif(j1.gt.0.and.j2.gt.0) then          !  < kl ... | ... kl>
       ...
       if(iext.gt.1) Call Stop_mpi(0,0,' Check_det: iext > 1')
        ...
        Do j=1,kdn
         ...
         if(j.eq.j2) then
          kdn1=kdn-1
          if(io.eq.0) Call Stop_mpi(0,0,' Orthogonal kl ?')
