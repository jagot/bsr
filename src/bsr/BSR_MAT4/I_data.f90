...
! ... prepare B_spline representation and define symmetries:
      ...
      Select case(icase)
       ...
       Case Default; Call Stop_mpi(0,0,'unknown case in I_data')
      End Select
      ...
      Case(1)
        ...
        !  w = v_ch(:,k)
         ...
         elseif(ic.lt.0.and.jc.eq.0) then            ! R(..) <i|j>
          ...
          Call Stop_mpi(0,0,' I_data: unknown structure for itype=1')
        ...
        Do j=1,ncdata; i=IPT(j); jj=k1(i); ii=k2(i); ich=k3(i); io=k4(i)
         ...
         if(jj.ne.jjj.or.ii.ne.iii) then
          ...
          if(jj.ne.jjj) then
           ...
           Call Density(ns,ks,dd,pbs(1,j1),pbs(1,j2),sym_d)
           Call Convol(ns,ks,xx,dd,jcase,sym_i,sym_j)
           jjj = jj
          end if
...
!                                                      RK ( i . ; j . )
      ...
      Case Default
       ...
       Call Stop_mpi(0,0,' I_data: unknown itype ')
