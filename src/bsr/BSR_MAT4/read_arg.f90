...
!----------------------------------------------------------------------
      Use bsr_mat, nb=>nblock, mb=>mblock, kb=>kblock
...
! ... read parameters from file if any
      ...
      Call Read_ipar(nu,'ipert_ch',ipert_ch)
      Call Read_ipar(nu,'bp_mode',bp_mode)
      Call Read_ipar(nu,'exch_mode',exch_mode)
      Call Read_ipar(nu,'mode',mode)
      ...
      Call Read_rpar(nu,'time',time_delay)
...
! ... overwrite parameters from arguments if any
      ...
      Call Read_iarg('ipert_ch',ipert_ch)

      Call Read_iarg('bp_mode'  ,bp_mode)
      Call Read_iarg('exch_mode',bp_mode)
      Call Read_iarg('mode'     ,mode)

      if(exch_mode.eq.1) check_target=0
      if(exch_mode.eq.2) bp_mode=1
      ...
      Call Read_rarg('time',time_delay)
...
! ... other parameters:
      ...
      write(prj,'(a,i7,a)') 'nb      =',nb, &
       ' - number of blocks in module c_data'
