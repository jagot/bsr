...
!----------------------------------------------------------------------
      ...
      Use bsr_mat
      Use conf_LS
      Use Ndet_list, only: ndet; Use Ndef_list, only: ndef
...
! ... send the final coefficients to archive:
      ...
      if( time_limit.gt.0.d0 .and. (tt2-t0)/60.gt.time_limit*1.1) then
