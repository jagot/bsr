...
! ... read parameters from file if any
      ...
      Call Read_ipar(nu,'iexp_pert',iexp_pert)
...
! ... overwrite parameters from arguments if any
      ...
      Call Read_iarg('iexp_pert',iexp_pert)
...
! ... read corrections for pertuber:
      ...
      if(iexp_pert.gt.0) then
       ...
       if(nperts.eq.0) Stop 'iexp_pert > 0  but npert = 0'
       Call Read_apar(nue,'unit',unit)
       Allocate(E_npert(nperts), E_pert_corr(nperts))
       rewind(nue)
       Do i=1,nperts
        read(nue,*) AF, E_npert(i), E_pert_corr(i)
       End do
       unit = 'eV'
       if(unit.eq.'eV') E_pert_corr = E_pert_corr / au_eV
       if(unit.eq.'cm') E_pert_corr = E_pert_corr / au_cm
      end if
