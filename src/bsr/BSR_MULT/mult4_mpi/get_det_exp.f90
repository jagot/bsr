...
!======================================================================
      ...
      Use mult_par,  only: ierr,JT_oper,CT_oper
      Use conf_LS,   only: ne
      ...
      Call MPI_RECV(MLT1,1,MPI_INTEGER,0,0,MPI_COMM_WORLD, status,ierr)
      Call MPI_RECV(MST1,1,MPI_INTEGER,0,0,MPI_COMM_WORLD, status,ierr)

      Call MPI_RECV(MLT2,1,MPI_INTEGER,0,0,MPI_COMM_WORLD, status,ierr)
      Call MPI_RECV(MST2,1,MPI_INTEGER,0,0,MPI_COMM_WORLD, status,ierr)
      ...
      if(allocated(JT_oper)) Deallocate(JT_oper); Allocate(JT_oper(ntrm))
      Call MPI_RECV(JT_oper,ntrm,MPI_INTEGER,0,0,MPI_COMM_WORLD, status,ierr)
      if(allocated(CT_oper)) Deallocate(CT_oper); Allocate(CT_oper(ntrm))
