...
! ... L-integrals <P_i| L |P_j>:

      Real(8), allocatable :: L_int(:,:)
...
!---------------------------------------------------------------------
      ...
      if(ns.gt.0) then
       ...
       Allocate(hl(ns,ks), hl_dir(ns,ks), hl_exc(ns,ns), hl_core(ns,ns), &
                L_int(nbf,nbf)  )
       ...
       Deallocate(hl,hl_dir,hl_exc,hl_core, L_int)
       lh_dir = -1
...
! ... generate L-data for given l:
      ...
      Do l=0,mlh
       ...
       Do i = kclosd+1,nbf;  if(lbs(i).ne.l) Cycle
        Do j = i,nbf;        if(lbs(j).ne.l) Cycle
...
!---------------------------------------------------------------------
      ...
      Real(8) :: C1, C2, x(ns,ks), xx(ns,ns), vc(ns,ks)
      Character(1) :: sym
      Real(8), external :: ZCB
