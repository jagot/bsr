...
!----------------------------------------------------------------------
      ...
      Integer :: jc1,jc2, iarg, ip1,ip2, ibuf, ncbuf
      ...
      if(iarg.lt.1.or.name.eq.'?') then
       ...
       write(*,*) 'bnk=...  -  bnk-file with inforation   [int_inf]'
...
! ... read the specific data:
   ...
   10 read(nui,end=20) ncbuf
write(*,*) 'ncbuf',ncbuf
      if(ncbuf.eq.0) Stop 'ncbuf = 0'
      ...
      it=k1(ibuf); jt = k2(ibuf); int = k3(ibuf)
