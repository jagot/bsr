!=======================================================================
      Subroutine Coef_ee_2conf(no1,nn1,ln1,iq1,LS1,no2,nn2,ln2,iq2,LS2, &
                               mcoef, ncoef, icoefs, coefs)
!=======================================================================
!     compute the angular coefficients between 2 atomic states
!----------------------------------------------------------------------
      Implicit none
...
! ... input-output:
      ...
      Integer, intent(out) :: ncoef, icoefs(5,mcoef)
      Real(8), intent(out) :: coefs(mcoef)
...
! ... local variables:
      ...
      Integer :: ne, kd1, kd2, i,j, k, mdt1, mdt2, kmax, i1,i2,j1,j2
      Integer, allocatable :: nl1(:), nl2(:), ip1(:), ip2(:)
      Real(8), allocatable :: coef(:,:,:,:,:)
      Integer, external :: Iglq

      ncoef = 0
      if(LS1(4,no1).ne.LS2(4,no2)) Return
...
! ... calculations:
      ...
      kmax = maxval(ln1(1:no1)) + maxval(ln2(1:no2))
...
! ... find interaction orbitals:
       ...
       Case(0)
        ...
        Do i=1,ne; k1=ip1(i); k2=ip2(i)
         coef(k1,k1,k2,k2,-1) = coef(k1,k1,k2,k2,-1) + CC_det
        End do

        Do i1=1,ne-1; Do i2=i1+1,ne
         Call me_ee(i1,i2,i1,i2)
        End do; End do

      Case(1)  ! "AFTER" conditios supposed

        i = ii(1); j = jj(1)
        k1=ip1(i); k2=ip2(j)
        if(MLdet1(i,kd1).eq.MLdet2(j,kd2).and. &
           MSdet1(i,kd1).eq.MSdet2(j,kd2).and. &
           ln1(k1).eq.ln2(k2)) then
         coef(k1,k1,k2,k2,-1) = coef(k1,k1,k2,k2,-1) + CC_det * (-1)**(i+j)
        end if

        i1 = 0; j1 = 0; i2=ii(1); j2=jj(1)
        Do i=1,ne-1
         i1=i1+1; if(i1.eq.i2) i1=i1+1
         j1=j1+1; if(j1.eq.j2) j1=j1+1
         k1=i1;k2=i2; if(i1.gt.i2) then; k1=i2; k2=i1; end if
         k3=j1;k4=j2; if(j1.gt.j2) then; k3=j2; k4=j1; end if
         Call me_ee(k1,k2,k3,k4)
        End do
       Case(2)
        Call me_ee(ii(1),ii(2),jj(1),jj(2))
       End Select
...
!----------------------------------------------------------------------
      ...
      if(MLdet1(i1,kd1) + MLdet1(j1,kd1).ne. &
         MLdet2(i2,kd2) + MLdet2(j2,kd2)) Return
      if(MSdet1(i1,kd1) + MSdet1(j1,kd1).ne. &
         MSdet2(i2,kd2) + MSdet2(j2,kd2)) Return

      n1=ip1(i1); n2=ip1(j1); n3=ip2(i2); n4=ip2(j2)

      Call Check_boef(ln1(n1),MLdet1(i1,kd1),MSdet1(i1,kd1), &
                      ln1(n2),MLdet1(j1,kd1),MSdet1(j1,kd1), &
                      ln2(n3),MLdet2(i2,kd2),MSdet2(i2,kd2), &
                      ln2(n4),MLdet2(j2,kd2),MSdet2(j2,kd2))
