...
!----------------------------------------------------------------------
      Use bsr_breit,     only: jort, is_conf, js_conf
      Use conf_LS,       only: ne
...
! orthogonality insert:           ???

       if(jort.eq.-1.and.is_conf.eq.js_conf) then
        Do i=1,k1
         if(N1(i).eq.N2(i)) Cycle
         kd = -1
         Exit
        End do
        if(kd.eq.-1) Exit
      ...
      Idet_fact=kd; if(kd.le.0) Return
