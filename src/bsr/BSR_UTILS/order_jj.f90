...
!                                                           input data:
      ...
      if(iarg.lt.1.or.AF.eq.'?') then
        write(*,*)
        write(*,*) 'ORDER_JJ: ordering the ASF according to their weights'
        write(*,*)
      ...
      if(iarg.gt.1) then; Call GETARG(2,BF); read(BF,*) eps_c; end if

!----------------------------------------------------------------------
! ... read list of configuration from c-file:

      i=INDEX(AF,'.',BACK=.TRUE.)
      if(AF(i+1:i+1).ne.'c') Stop ' c-file should have extensions c'
      Call Check_file(AF)
      Open(nuc,file=AF)
