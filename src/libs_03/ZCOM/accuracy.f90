...
module accuracy
  implicit none
  private
  public sik, ik, hik, rk, xk, xrk, ark
  public input, out
  public clen
  public srk, drk, lrk
  public qrk
!*qd  public qrk
  public rk_bytes, sik_bytes, ik_bytes, hik_bytes, xk_bytes, xrk_bytes
  public safe_max, max_exp
  public flush_wrapper
  public isnan_wrapper
  public rcsid_accuracy
  !
  integer, parameter :: sik    = selected_int_kind(4)       ! "Short" integers
...
!*qd  integer, parameter :: xrk         = selected_real_kind(28,50)  ! "Highly-accurate" reals and complex
  ...
  !
  character(len=clen), save :: rcsid_accuracy = "$Id: accuracy.f90,v 1.6 2021/11/13 14:57:26 ps Exp $"
  !
...
!*qd module procedure isnan_quad
!*qd module procedure isnan_quad_complex
  ...
  elemental function isnan_quad(x) result(v)
    ! Depending on the compiler, it may be necessary to uncomment one of the lines below
      use IEEE_ARITHMETIC , only: isnan => ieee_is_nan ! This may be very expensive - it keeps switching FPU mode flags.
    ! intrinsic            :: isnan                    ! This works for relatively recent gfortran and ifort versiobns
    ! logical, external    :: isnan
    real(xrk), intent(in) :: x
    logical               :: v
    !
    v = isnan(x)
  end function isnan_quad
  ...
  elemental function isnan_quad_complex(x) result(v)
    complex(xrk), intent(in) :: x
    logical                  :: v
    !
    v = isnan_quad(real(x,kind=xrk)) .or. isnan_quad(aimag(x))
  end function isnan_quad_complex
