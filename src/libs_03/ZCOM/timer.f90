module Timer
  !  These timing routines come courtesy of Dr. Serguei Patchkovskii
  !  (serguei.patchkovskii@mbi-berlin.de), and have been slightly
  !  adapted.
  !
...
contains
  ...
  !
  subroutine TimerReport(print_percentages)
    logical, optional  :: print_percentages
    logical            :: print_percentages_
    real(trk)          :: real_now
    real(trk)          :: cpu_now
    real(trk)          :: real_time, cpu_time
    real(trk)          :: real_kids, cpu_kids
    real(trk)          :: real_time_cumul, cpu_time_cumul
    ...
    if (omp_secondary()) return ! Timer routines are not thread-safe otherwise!
    !
    print_percentages_ = .false.
    if(present(print_percentages)) print_percentages_ = print_percentages
    !
    real_now  = get_real_time()
    cpu_now   = get_cpu_time()
    timestamp = get_timestamp()
    !
    real_threshold = 0.0_trk * (real_now - prog_start)
    cpu_threshold  = 0.0_trk * cpu_now
    !
    ...
    if(.not.print_percentages_) then
       ...
       write (out,"(t2,'     ',t42,'     ',t56,'Total time (seconds)',t103,'Self time (seconds)')")
       write (out,"(t2,'Timer',t42,'Calls',t56,a,t103,a)") repeat("-", 37), repeat("-", 36)
       write (out,"(t2,'-----',t42,'-----',t58,'Real',t73,'CPU',t86,'Speedup',t104,'Real',t119,'CPU',t132,'Speedup')")
    end if
    write (out,"()")
    !
    omitted = 0
    scan: do ord=1,t_count
       ...
       real_time_cumul = real_time_cumul + real_time
       cpu_time_cumul  = cpu_time_cumul + cpu_time
       !
       !  Output needed?
       !
       ...
       if(.not.print_percentages_) then
          ...
          write (out,"(t2,a30,t33,a1,t35,f12.0,t49,2(f13.1,1x,f13.1,3x,f13.1,'%',2x))") &
               t%name, active, t%calls, &
               real_time, cpu_time, 100.0_rk*cpu_time/real_time, &
               real_time - real_kids, cpu_time - cpu_kids, &
               100.0_rk*(cpu_time - cpu_kids)/(real_time - real_kids)
       end if
    end do scan
    if (omitted>0) then
